...
export default function DocumentTypeControl({ documentId, currentType }: { documentId: string; currentType: string | null }) {
  ...
  async function confirmType() {
    ...
    try {
      ...
      if (!response.ok) { setMessage(result?.error ?? "Het type kon niet veilig worden bevestigd. Er is niets als definitief opgeslagen."); return; }
      setMessage("Type bevestigd."); router.refresh();
    } catch { setMessage("De verbinding werd onderbroken. Probeer opnieuw."); }
    finally { setBusy(false); }
  }

  return <div className="document-type-control"><label htmlFor={`document-type-${documentId}`}>Wat voor document is dit?</label><div className="document-type-control-row"><select id={`document-type-${documentId}`} value={value} disabled={busy} onChange={(event) => { setValue(event.target.value as DocumentType | ""); setMessage(null); }}><option value="">Kies een type</option>{documentTypes.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select><button className="button secondary" type="button" disabled={!value || busy} onClick={() => void confirmType()}>{busy ? "Opslaan…" : currentType ? "Type wijzigen" : "Type bevestigen"}</button></div><span className="muted" aria-live="polite">{message ?? "Kies alleen wat je zelf herkent. Een factuur hoort via Facturen en kan hier niet als ander document worden bevestigd."}</span></div>;
}
