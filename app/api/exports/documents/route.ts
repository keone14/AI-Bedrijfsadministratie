...
function uniqueFilename(filename: string, used: Set<string>) {
  if (!used.has(filename.toLocaleLowerCase("nl-BE"))) {
    used.add(filename.toLocaleLowerCase("nl-BE"));
    return filename;
  }
  ...
  let counter = 2;
  while (used.has(`${stem} (${counter})${extension}`.toLocaleLowerCase("nl-BE"))) counter += 1;
  const candidate = `${stem} (${counter})${extension}`;
  used.add(candidate.toLocaleLowerCase("nl-BE"));
  return candidate;
}
...
function makeStoredZip(files: Array<{ filename: string; data: Uint8Array }>) {
  ...
  for (const file of files) {
    ...
    entries.push({ name, data: file.data, crc: crc32(file.data), offset: localSize });
    localSize += 30 + name.length + file.data.length;
  }
...
export async function GET() {
  ...
  if (membershipError) return errorResponse("We konden je bedrijfsrechten nu niet betrouwbaar controleren. Probeer opnieuw.", 503);
  if (!memberships?.length) return errorResponse("Stel eerst je bedrijf in voordat je documenten exporteert.", 409);
  if (memberships.length > 1) return errorResponse("Kies eerst één actief bedrijf. We exporteren nooit documenten van meerdere bedrijven door elkaar.", 409);
  ...
  for (const document of documents) {
    ...
    const fallback = `document-${document.id}`;
    const filename = uniqueFilename(safeFilename(document.original_filename, fallback), usedNames);
    files.push({ filename, data: bytes });
  }
  ...
  const body = new Blob([zip.buffer.slice(zip.byteOffset, zip.byteOffset + zip.byteLength)], { type: "application/zip" });
  const response = new NextResponse(body, { status: 200 });
  response.headers.set("Content-Type", "application/zip");
  response.headers.set("Content-Disposition", `attachment; filename="originele-documenten-${today}.zip"`);
