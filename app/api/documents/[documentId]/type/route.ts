...
export async function POST(request: Request, { params }: { params: Promise<{ documentId: string }> }) {
  ...
  const { data: document, error: documentError } = await supabase.from("documents").select("id").eq("id", documentId).eq("company_id", company.companyId).maybeSingle();
  ...
  const { error } = await supabase.rpc("set_document_type", { target_document_id: documentId, target_document_type: parsed.data.type });
