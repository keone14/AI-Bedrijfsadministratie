...
function processingConfiguration(): ProcessingConfiguration | null {
  ...
  if (provider === "openai") { const apiKey = process.env.OPENAI_API_KEY; const model = process.env.OPENAI_INVOICE_MODEL; if (!apiKey || !model) return null; return { provider, apiKey, model }; }
  if (provider === "fixture" && process.env.ALLOW_SYNTHETIC_AI_FIXTURE === "true" && process.env.VERCEL_ENV !== "production") return { provider, model: "synthetic-fixture-v1" };
  return null;
}
...
export async function GET() {
  ...
  return NextResponse.json({ enabled: Boolean(configuration), provider: configuration?.provider ?? null, synthetic: configuration?.provider === "fixture" });
}
...
export async function POST(_request: Request, context: RouteContext) {
  ...
  after(async () => {
    try {
      ...
      if (configuration.provider === "fixture") { extraction = extractSyntheticInvoiceFixture(); method = "synthetic_fixture_no_external_data"; }
        ...
        const { data: blob, error: downloadError } = await admin.storage.from("company-documents").download(document.storage_path);
        if (downloadError || !blob) throw new Error("document download failed");
        extraction = await extractInvoiceWithOpenAI({ apiKey: configuration.apiKey, model: configuration.model, buffer: Buffer.from(await blob.arrayBuffer()), mimeType: document.mime_type as "application/pdf" | "image/jpeg" | "image/png", filename: document.original_filename, companyName: company.name });
  ...
  return NextResponse.json({ invoiceId: invoice.id, jobId, status: "processing", synthetic: configuration.provider === "fixture", message: configuration.provider === "fixture" ? "Synthetische stagingtest gestart. Er wordt geen documentinhoud naar een externe AI-dienst gestuurd." : "De factuur wordt veilig uitgelezen. Het resultaat wordt altijd eerst als te controleren gegevens opgeslagen." }, { status: 202 });
}
