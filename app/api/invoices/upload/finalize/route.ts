...
export async function POST(request: Request) {
  ...
  if (fileBlob.size < 1 || fileBlob.size > MAX_FILE_SIZE) { await discard(supabase, storagePath); return NextResponse.json({ error: "Een factuur mag maximaal 10 MB groot zijn." }, { status: 400 }); }

  const buffer = Buffer.from(await fileBlob.arrayBuffer());
  const detectedMime = detectMime(buffer);
  if (!detectedMime) { await discard(supabase, storagePath); return NextResponse.json({ error: "Het bestand is geen geldige PDF-, JPG- of PNG-factuur." }, { status: 400 }); }
  if (expectedMimeByExtension[storageExtension] !== detectedMime) { await discard(supabase, storagePath); return NextResponse.json({ error: "De bestandsnaam en de werkelijke inhoud komen niet overeen. Exporteer of hernoem de factuur als PDF, JPG of PNG en probeer opnieuw." }, { status: 400 }); }
  ...
  if (registerError) { await discard(supabase, storagePath); const duplicate = registerError.message?.toLowerCase().includes("duplicate document"); return NextResponse.json({ error: duplicate ? "Deze factuur lijkt exact hetzelfde bestand te zijn als een factuur die al is opgeslagen." : "De factuur kon niet betrouwbaar worden geregistreerd." }, { status: duplicate ? 409 : 400 }); }
