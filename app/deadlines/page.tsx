...
function dateOnlyUtc(value: string) { const match = /^(\d{4})-(\d{2})-(\d{2})$/.exec(value.slice(0, 10)); if (!match) return null; const year = Number(match[1]); const month = Number(match[2]); const day = Number(match[3]); const date = new Date(Date.UTC(year, month - 1, day)); if (date.getUTCFullYear() !== year || date.getUTCMonth() !== month - 1 || date.getUTCDate() !== day) return null; return date.getTime(); }
...
function formatDate(value: string) { const time = dateOnlyUtc(value); if (time === null) return "Datum niet betrouwbaar"; return new Intl.DateTimeFormat("nl-BE", { timeZone: "Europe/Brussels", day: "numeric", month: "long", year: "numeric" }).format(new Date(time + 43_200_000)); }
function formatAmount(total: number | null, currency: string | null) { if (total === null || !Number.isFinite(Number(total))) return "Bedrag niet bevestigd"; const amount = Number(total); const normalizedCurrency = (currency ?? "").trim().toUpperCase(); if (/^[A-Z]{3}$/.test(normalizedCurrency)) { try { return new Intl.NumberFormat("nl-BE", { style: "currency", currency: normalizedCurrency }).format(amount); } catch { /* fall through to a neutral amount */ } } const formatted = new Intl.NumberFormat("nl-BE", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(amount); return normalizedCurrency ? `${formatted} ${normalizedCurrency}` : formatted; }
...
function actionLabel(invoice: InvoiceRow) { if (invoice.invoice_type === "purchase") return "Aankoopfactuur · betaling controleren"; if (invoice.invoice_type === "sale") return "Verkoopfactuur · ontvangst controleren"; return "Factuur · actie controleren"; }
