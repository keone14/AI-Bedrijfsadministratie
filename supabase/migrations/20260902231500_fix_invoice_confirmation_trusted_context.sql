...
-- Confirmation is an authenticated, narrowly-scoped RPC. It must be allowed through
-- the server-managed invoice/document state triggers, while the approval evidence
-- trigger still verifies that approved_by equals auth.uid() and stamps approved_at.
